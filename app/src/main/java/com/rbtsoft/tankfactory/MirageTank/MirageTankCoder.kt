...
object MirageTankCoder {
    ...
    suspend fun encode(photo1: Bitmap, photo2: Bitmap, photo1K: Float, photo2K: Float, threshold: Int): Bitmap =
        withContext(Dispatchers.Default) {
            ...
            val scaledPhoto1 = if (photo1.width != width || photo1.height != height) { photo1.scale(width, height) } else { photo1 }
            val scaledPhoto2 = if (photo2.width != width || photo2.height != height) { photo2.scale(width, height) } else { photo2 }
            ...
            if (scaledPhoto1 !== photo1) { scaledPhoto1.recycle() }
            if (scaledPhoto2 !== photo2) { scaledPhoto2.recycle() }
