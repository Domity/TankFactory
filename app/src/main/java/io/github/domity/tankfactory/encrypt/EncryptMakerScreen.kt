...
fun EncryptMakerScreen(
    ...
    val onSelectFileClick = remember { { filePickerLauncher.launch("image/*") } }
    ...
    CyberScaffold(useSafeArea = true) {
        Column(
            ...
            CyberButton(
                text = stringResource(id = if (isProcessing) R.string.encrypt_maker_encrypting else R.string.encrypt_maker_encrypt),
...
internal fun FileSelectionSlot(
    ...
    val isFileSelected = fileName != null
    CyberSurface(
        ...
        Box(
            ...
            CyberText(
                text = fileName ?: placeholderText,
                color = if (isFileSelected) CyberTheme.colors.text else CyberTheme.colors.textDim,
                style = CyberTheme.typography.body
            )
        }
    }
