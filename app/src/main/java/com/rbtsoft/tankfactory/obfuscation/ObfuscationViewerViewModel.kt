...
class ObfuscationViewerViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun decrypt() {
        ...
        viewModelScope.launch(Dispatchers.Default) {
            val app = getApplication<Application>()
            try {
                val result = ObfuscationCoder.decrypt(data, pwd)
                if (result != null) {
                    decryptedData = result
                    _isDone.value = true
                } else {
                    decryptedData = null
                    _errorMessage.value = app.getString(R.string.obfuscation_viewer_decrypt_failed)
                }
            } catch (_: Exception) {
                _errorMessage.value = app.getString(R.string.obfuscation_viewer_decrypt_failed)
            } finally {
